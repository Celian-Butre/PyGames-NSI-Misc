...
def checkNeighbourOnFire(coord_i,coord_j, Liste_Cases, Meteo_Extractee):
    neighboursOnFire = []
                ...
                if (i != 1) or (j != 1):
                    ...
                    if IntensiteActuelle != 0:
                        ...
                        if Meteo_Extractee == "Pluie" or Meteo_Extractee == "Foudre":
                            ...
                            InfluenceMeteo = -0.25
                        ProbabiliteFeu = ProbabiliteFeu - ProbabiliteFeu * InfluenceMeteo
...
def La_Propagation_Feu(Liste_Cases, Meteo_Extractee):
    Liste_Cases_Renfort = copy.deepcopy(Liste_Cases)
    for i in range (len(Liste_Cases)):
        for j in range (len(Liste_Cases)):
            if Liste_Cases[i][j]["etat"] == "Vierge" and Liste_Cases[i][j]["Biome"] != 2:
                neighboursOnFire = checkNeighbourOnFire(i, j, Liste_Cases_Renfort, Meteo_Extractee)
